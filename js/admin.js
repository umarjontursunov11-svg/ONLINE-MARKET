...
class AdminManager {
  ...
  // Bot tokeni va guruh ID faqat serverda (Vercel Environment Variables:
  // TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID). Admin paneldan faqat sinov xabari yuboriladi.
  initTelegramSettings() {
    // Eski versiyalarda brauzerga saqlangan tokenni o'chiramiz
  ...
  async testTelegramGroup() {
    ...
    const testMsg = `🧪 <b>TEST XABARI — STANDART VA METROLOGIYA TIZIMI</b>\n` +
      `━━━━━━━━━━━━━━━━━━━━━━\n` +
      `✅ Saytdagi buyurtmalar integratsiyasi server orqali ishlayapti.\n` +
    ...
    try {
      ...
      if (res.ok && data.ok) {
        ...
        alert("⚠️ Telegram sozlanmagan.\n\nVercel → Settings → Environment Variables bo'limiga TELEGRAM_BOT_TOKEN va TELEGRAM_CHAT_ID qo'shing, so'ng saytni qayta joylang (Redeploy).");
      } else {
        alert(`⚠️ Xabar yuborilmadi (${res.status}): ${data.message || ''}${data.code ? ' — Telegram kodi ' + data.code : ''}\n\nBot guruhga qo'shilgan va xabar yozish ruxsatiga ega ekanini tekshiring.`);
      }
    } catch (e) {
